...
function getUserAddress(): string | undefined {
  ...
  try {
    ...
    const sessionData = localStorage.getItem('starknomo_starknet_wallet_session');
    if (sessionData) {
...
function getSessionId(): string {
  ...
  try {
    let sessionId = sessionStorage.getItem('starknomo_session_id');
    if (!sessionId) {
...
function storeErrorLocally(errorLog: ErrorLogEntry): void {
  ...
  try {
    const key = 'starknomo_error_logs';
...
export function clearStoredErrorLogs(): void {
  ...
  try {
    localStorage.removeItem('starknomo_error_logs');
