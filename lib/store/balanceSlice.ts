...
export const createBalanceSlice: StateCreator<BalanceState> = (set, get) => ({
  ...
  fetchBalance: async (address: string) => {
    ...
    // STRK-only: always use STRK as currency
    const network = 'STRK';
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = rawText?.trim().startsWith('<') ? `Server error (${response.status})` : (rawText || `Request failed (${response.status})`);
        }
        throw new Error(errorMessage);
      }

      let data: { newBalance?: number; txHash?: string };
      try {
      ...
      // Validate newBalance is a valid number before setting
      if (data.newBalance === undefined || data.newBalance === null) {
        console.error('Invalid withdrawal response: newBalance is missing', { data });
        throw new Error('Failed to process withdrawal: invalid response from server');
      }

      set({
        houseBalance: Number(data.newBalance),
        isLoading: false,
        error: null
      });
