 ...
 * 
 * Note: Bet history is managed off-chain on Starknet.
...
export const createHistorySlice: StateCreator<HistoryState> = (set, get) => ({
  ...
  clearHistory: () => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.removeItem('starknomo_bet_history');
    }
  },
...
const saveBetsToLocalStorage = (bets: BetRecord[]): void => {
  try {
    if (typeof window !== 'undefined') {
      localStorage.setItem('starknomo_bet_history', JSON.stringify(bets));
    }
  } catch (error) {
...
const loadBetsFromLocalStorage = (): BetRecord[] => {
  try {
    if (typeof window !== 'undefined') {
      const data = localStorage.getItem('starknomo_bet_history');
      if (data) {
