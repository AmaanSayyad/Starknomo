 ...
 * 
 * Note: After Starknet migration, game logic remains off-chain.
...
export interface GameState {
  ...
  lastResult: {
    ...
    currency?: string; // The currency usage (e.g. STRK)
  } | null;
  error: string | null;
...
export const createGameSlice: StateCreator<any> = (set, get) => ({
   ...
   * Note: After Starknet migration, this method is deprecated.
   * Use placeBetFromHouseBalance instead for off-chain betting.
   * @param amount - Bet amount in STRK tokens (e.g., "1.0")
   * @param targetId - ID of the target cell (1-8) OR dynamic grid target (e.g., "UP-2.50")
   */
  placeBet: async (amount: string, targetId: string) => {
    throw new Error("placeBet is deprecated after Starknet migration. Use placeBetFromHouseBalance instead.");
  },
   ...
   * @param amount - Bet amount in STRK tokens
  ...
  placeBetFromHouseBalance: async (amount: string, targetId: string, userAddress: string, cellId?: string, metadata?: { priceTop?: number; priceBottom?: number; endTime?: number }) => {
    ...
    try {
      ...
      // Get current network from store (defaults to STRK)
      const network = (get() as any).network || 'STRK';
      ...
      const activeBet: ActiveBet = {
        ...
        timestamp: Date.now(),
        status: 'active',
        network: network, // Save the network used (STRK)
        ...(gameMode === 'binomo' ? {
          strikePrice: currentPrice,
          endTime: Date.now() + (durationSeconds * 1000)
        } : {
   ...
   * Note: After Starknet migration, settlement is handled automatically by the instant-resolution system.
   * This method is kept for backward compatibility but does nothing.
   * @param betId - The unique bet ID to settle
   */
  settleRound: async (betId: string) => {
    console.log('settleRound called but is deprecated after Starknet migration');
  ...
  updatePrice: (price: number, asset?: AssetType) => {
    ...
    activeBets.forEach((bet: ActiveBet) => {
      ...
      const betAsset = bet.asset || 'STRK'; // Fallback to STRK
      if (betAsset !== currentSelectedAsset || bet.status !== 'active') return;

      // Starknomo (Classic) Logic
   ...
   * Note: Target cells are configured off-chain (no chain query).
    ...
    if (resolvedBet) {
      ...
      set({
        ...
        lastResult: {
          ...
          currency: resolvedBet.network || (network || 'STRK')
        }
      });
      ...
      if (won) {
        if (accountType === 'real' && address) {
          void (async () => {
            try {
              const res = await fetch('/api/balance/win', {
                method: 'POST',
              ...
              if (!res.ok) {
                ...
                console.error(`Failed to credit win balance: ${res.status} ${res.statusText} - ${errorText}`);
                return;
              }

              if (fetchBalance) await fetchBalance(address);
            } catch (error) {
              console.error('Failed to credit win balance:', error);
            }
          })();
