...
describe('user_balances table schema', () => {
  ...
  describe('Edge cases', () => {
    it('should handle Starknet address format correctly', async () => {
      const starknetAddresses = [
        '0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef',
        '0xf8d6e0586b0a20c7f8d6e0586b0a20c7f8d6e0586b0a20c7f8d6e0586b0a20c7',
        '0x0000000000000001000000000000000100000000000000010000000000000001',
      ];

      for (const address of starknetAddresses) {
        const { data, error } = await supabase
          .from('user_balances')
          .insert({
            user_address: address,
