...
export const BalanceDisplay: React.FC = () => {
  const houseBalance = useStarknomoStore(state => state.houseBalance);
  const demoBalance = useStarknomoStore(state => state.demoBalance);
  const accountType = useStarknomoStore(state => state.accountType);
  const network = useStarknomoStore(state => state.network);
  const toggleAccountType = useStarknomoStore(state => state.toggleAccountType);
  const isLoading = useStarknomoStore(state => state.isLoading);
  const address = useStarknomoStore(state => state.address);
  ...
  const setAddress = useStarknomoStore(state => state.setAddress);
  const setIsConnected = useStarknomoStore(state => state.setIsConnected);
  ...
  return (
    <>
      <div className="bg-black/30 rounded-xl border border-white/5 overflow-hidden">
        {/* Secret Demo Header - Only visible when activated via Starknomo logo click */}
        {accountType === 'demo' && (
