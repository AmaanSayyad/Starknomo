 ...
 * Displays a Starknomo-style popup when a bet is settled (win or loss)
 */
export const SettlementNotification: React.FC = () => {
    const lastResult = useStarknomoStore(state => state.lastResult);
    const clearLastResult = useStarknomoStore(state => state.clearLastResult);
                        ...
                        `}>
                            ...
                            <div className="flex flex-col">
                                <span className="text-[10px] font-black uppercase tracking-tighter leading-none opacity-60">
                                    {(lastResult.currency || 'STRK')} {lastResult.won ? 'PROFIT' : 'LOSS'}
                                </span>
                                <span className="text-sm font-mono font-black tracking-tight leading-tight">
                                    {lastResult.won ? '+' : '-'}{Math.abs(lastResult.won ? lastResult.payout : lastResult.amount).toFixed(4)} <span className="text-[10px] opacity-70">{lastResult.currency || 'STRK'}</span>
                                </span>
                            </div>
