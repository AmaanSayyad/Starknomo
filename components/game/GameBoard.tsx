...
export const GameBoard: React.FC = () => {
  ...
  const handleEnterBlitz = async () => {
    ...
    try {
      ...
      toast.info(`Preparing ${blitzEntryFee} STRK Blitz Entry...`);

      const treasuryAddress = starknetSepolia.treasuryAddress;
      if (!treasuryAddress) throw new Error('Treasury address not configured');

      const hash = await sendTransfer(treasuryAddress, blitzEntryFee.toString());
  ...
  const handleStarknomoBet = async (direction: 'UP' | 'DOWN') => {
                ...
                <div className="space-y-4">
                  ...
                  {gameMode === 'binomo' ? (
                    <div className="grid grid-cols-2 gap-3 pt-2">
                      ...
                      <button
                        onClick={() => handleStarknomoBet('DOWN')}
                        disabled={!isWalletConnected || isPlacingBet || isUnauthorized}
                  ...
                  <div className={`rounded-xl p-4 relative overflow-hidden border-2 transition-all duration-500 ${isBlitzActive ? 'bg-gradient-to-br from-orange-500/20 via-red-500/10 to-yellow-500/10 border-orange-500/40 shadow-[0_0_20px_rgba(249,115,22,0.1)]' : 'bg-black/40 border-gray-800/50'}`}>
                    ...
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div className="space-y-3">
                        <div className="p-2 bg-black/40 rounded-lg border border-white/10 flex items-center justify-between">
                          <span className="text-[9px] uppercase tracking-wider text-gray-400 font-bold">Gas Mode</span>
                          <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded ${isSocialWallet ? 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/40' : 'bg-amber-500/20 text-amber-300 border border-amber-500/40'}`}>
