...
export async function POST(request: NextRequest) {
    try {
        ...
        const { userAddress, winAmount, currency = 'STRK', betId } = body;
        ...
        if (error) {
            console.error('Database error in win credit RPC, trying fallback:', error);
            ...
            const newBalance = Number((currentBalance + Number(winAmount)).toFixed(18));
